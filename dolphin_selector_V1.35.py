...
# ====================================================================
# 26.06 參數設定區 (V25.98視覺裝甲 × 方案2落難老兵 × 雷達明細全開)
# ====================================================================
...
def run_pre_backtest(api, stock_id):
    """🎯 升級版：極速 Numpy 體檢引擎，並回傳詳細明細供 HTML 渲染"""
...
<nav class="navbar navbar-dark px-4 py-3">
    <span class="navbar-brand mb-0 h1 fs-3">🐬 海豚量化自適應指揮官儀表板 <small class="fs-6 text-muted-custom">v26.06 資金周轉透視 × 雷達明細全開</small></span>
...
<div class="container-fluid p-4">
            ...
            <div class="card">
                ...
                <div class="table-responsive">
                    <table class="table table-custom-dark mb-0">
                        <thead>
                            <tr>
                                <th>代號/名稱</th><th>買入日期</th><th>成本價</th><th>目前價</th><th>起跑門檻</th>
    ...
    if not accordion_items:
        ...
        for sid, info in accordion_items.items():
            ...
            """
            if not records:
                html_content += f'<tr><td colspan="9" class="text-muted-custom py-3">⚠️ 未偵測到明細資料，可能為優化器外掛執行前已被攔截。</td></tr>'
            else:
...
async def main():
    print("====================================================")
    print("🚀 海豚選股 26.06：[V25.98視覺裝甲 × 雷達明細全開完全體] 啟動...")
    print("====================================================")
        ...
        try:
            ...
            if triggered_days_ago is not None and df.iloc[-1]["close"] >= max(df.iloc[-1]["5MA"], df.iloc[-1]["10MA"], df.iloc[-1]["20MA"]):
                ...
                if triggered_days_ago == 0:
                    ...
                    else: print(f"❌ [體檢失敗] {display_title} 回測期望值不佳，封殺！但明細已攔截供指揮官調閱。")
                ...
                if (max(today_ma) - min(today_ma)) / df.iloc[-1]["20MA"] <= MA_SPREAD_LIMIT:
                    ...
                    if stars == 3:
                        ...
                        if hist_p > 0:
                            ...
                            print(f"✅ [體檢過關] {display_title} 歷史期望值利潤：{hist_p} 元")
                        else: print(f"❌ [體檢失敗] {display_title} 封殺！明細已攔截供調閱。")
        except: pass
        time.sleep(0.01)
    ...
    # 🎯 終極修復：把剛剛雷達截留下來的「失敗新秀」明細，硬塞到優化器產生的 CSV 尾巴！
