...
# 🎯 【重磅升級：有限資金總池控管】
TOTAL_POOL_BUDGET = 250000   # 💰 你的實戰總資金上限為 25 萬
MIN_STOCK_BUDGET = 15000     # 🛡️ 防呆機制：單檔標的分配若低於 1.5 萬則不建倉，避免零股手續費不划算
...
def log_to_history_ledger(row, current_price, net_profit, profit_percent, exit_reason):
    ...
    if not os.path.exists(HISTORY_LEDGER_FILE):
        df_new.to_csv(HISTORY_LEDGER_FILE, index=False, encoding="utf-8-sig")
    else:
        df_new.to_csv(HISTORY_LEDGER_FILE, mode='a', header=False, index=False, encoding="utf-8-sig")

def run_pre_backtest(api, stock_id):
    """📝 25.85 改造：原本回傳 bool，現在回傳回測兩年的【總淨利金額】，若虧損則回傳 0"""
    ...
    try:
        ...
        for ma_opt in ma_options:
            for th in threshold_range:
                for dr in drop_range:
                    ...
                    # 為了回測標準化，這裡假設單檔使用固定的基準回測額度（30000）來量化評分
                    SIM_BASE = 30000
        ...
        return max(0, max_possible_profit) # 負數歸零，代表體檢失敗
    except:
        return 0
...
def update_and_print_portfolio(api, today_str):
    ...
    print("\n====================================================")
    print(f"💼 📊 【海豚選股 · 模擬帳戶當前資產即時回報 (有限資產控管版)】")
    print("====================================================")
    ...
    for idx, row in df_pf.iterrows():
        ...
        try:
            ...
            if not df_now.empty and len(df_now) >= 40: 
                # ===== 🎯 25.86 修正：精確提取收盤價與盤中最高價 =====
                current_price = float(df_now.iloc[-1]["close"])
                today_high = float(df_now.iloc[-1]["high"]) # 👈 導入盤中最高價
        ...
        # ===== 🎯 25.86 修正：用盤中最高價去更新波段最高點 =====
        max_price = float(row["max_price"]) if "max_price" in row and not pd.isna(row["max_price"]) else b_price
        if today_high > max_price:
            max_price = today_high # 👈 盤中只要有摸到新高，就算數！
            
        dynamic_lock_price = round(max_price * (1 - tp_dr), 2)
        
        # ===== 🎯 25.86 修正：只要歷史或今天盤中達標，且今日收盤破線，立刻攔截出場 =====
        if max_price >= target_tp_price and current_price <= dynamic_lock_price:
            exit_msg = f"🎉 鎖利通知：{sid} {sname} 曾創波段最高點 {max_price} (盤中已過起跑線 {target_tp_price})，今日收盤 {current_price} 已跌破鎖利防線 {dynamic_lock_price} (高點拉回 {tp_dr*100:.1f}%)！目前波段淨利: {sign}{net_profit}元 ({sign}{profit_percent:.2f}%)"
            print(exit_msg)
            exit_p_rows.append(exit_msg)
            log_to_history_ledger(row, current_price, net_profit, profit_percent, f"移動鎖利({tp_dr*100:.1f}%)")
            continue
        ...
        if active_stop_loss_value > 0.0 and current_price < active_stop_loss_value:
            ...
            print(exit_msg)
            exit_p_rows.append(exit_msg)
            log_to_history_ledger(row, current_price, net_profit, profit_percent, f"跌破均線({target_ma_line})")
            continue 
...
<nav class="navbar navbar-dark px-4 py-3">
    <span class="navbar-brand mb-0 h1 fs-3">🐬 海豚量化自適應指揮官儀表板 <small class="fs-6 text-muted-custom">v25.85 資金池優化版</small></span>
...
async def main():
    print("====================================================")
    print("🚀 海豚選股 25.85：[有限資產分流權重分配版] 啟動...")
    print("====================================================")
    ...
    # 🎯 【算出剩餘可用現金】
    available_cash = max(0, TOTAL_POOL_BUDGET - current_occupied_cash)
    print(f"💰 [資產總池] 限制上限: {TOTAL_POOL_BUDGET} 元 | 目前在庫持股已佔用: {current_occupied_cash:,.0f} 元")
    print(f"💰 [資產總池] 今日可用於購買新股的剩餘現金: {available_cash:,.0f} 元")

    # 用來暫存今天「通過體檢、準備分配權重」的新股票
    candidate_buys = []
            ...
            if triggered_days_ago is not None:
                if df.iloc[-1]["close"] >= max(df.iloc[-1]["5MA"], df.iloc[-1]["10MA"], df.iloc[-1]["20MA"]):
                    ...
                    if triggered_days_ago == 0 and stock not in sim_purchased_stocks and stock not in REMOVE_LIST:
                        print(f"🔬 偵測到發動標的：{display_title}，強行啟動 2年期望值精算體檢...")
                        hist_profit = run_pre_backtest(api, stock)
                        if hist_profit > 0:
                            k_line_date = str(df_raw.iloc[-1]["date"])[:10]
                            candidate_buys.append({
                                "stock_id": stock, "stock_name": c_name, "latest_close": latest_close,
                                "buy_type": "正飆(0天)", "buy_date": k_line_date, "score": hist_profit
                            })
                            print(f"✅ [體檢過關] {display_title} 兩年歷史最優解淨利：{hist_profit} 元（列入權重分配池）")
                        else:
                            print(f"❌ [體檢失敗] {display_title} 兩年歷史回測依然虧損或無解！直接封殺！")
                ...
                if f_5ma >= f_10ma >= f_20ma:
                    ...
                    if today_spread <= MA_SPREAD_LIMIT:
                        ...
                        if star_count == 3 and stock not in sim_purchased_stocks and stock not in REMOVE_LIST:
                            print(f"🔬 偵測到滿星起飆：{display_title}，強行啟動 2年期望值精算體檢...")
                            hist_profit = run_pre_backtest(api, stock)
                            if hist_profit > 0:
                                k_line_date = str(df_raw.iloc[-1]["date"])[:10]
                                candidate_buys.append({
                                    "stock_id": stock, "stock_name": c_name, "latest_close": latest_close,
                                    "buy_type": "3星起飆", "buy_date": k_line_date, "score": hist_profit
                                })
                                print(f"✅ [體檢過關] {display_title} 兩年歷史最優解淨利：{hist_profit} 元（列入權重分配池）")
                            else:
                                print(f"❌ [體檢失敗] {display_title} 兩年歷史回測依然虧損！拒慢建倉！")
    ...
    if candidate_buys and available_cash > MIN_STOCK_BUDGET:
        ...
        print(f"\n⚖️  [動態配比] 今日共選出 {len(candidate_buys)} 檔合格新股，總回測評分：{total_score}")
        
        for c in candidate_buys:
            # 依據回測利潤佔比算出權重
            weight = c["score"] / total_score
            # 配發預算 = 今日總剩餘可用現金 * 權重
            allocated_budget = available_cash * weight
            
            # 防呆：低於底線則不配發，避免買太少股手續費不划算
            if allocated_budget < MIN_STOCK_BUDGET:
                print(f"⚠️ {c['stock_id']}{c['stock_name']} 分配金額 {allocated_budget:,.0f} 低於最低限制，放棄建倉。")
                continue
            ...
            if calc_shares > 0:
                new_sim_buys.append({
                    ...
                    "buy_price": c["latest_close"], "buy_shares": calc_shares, 
                    "buy_type": c["buy_type"], "buy_date": c["buy_date"],
                    "best_tp": GLOBAL_TP_THRESHOLD, "best_drop": GLOBAL_TP_DROP, 
                    "best_ma": "20MA", "max_price": c["latest_close"] 
                })
                print(f"💰 [權重配發] 標的: {c['stock_id']} {c['stock_name']} | 權重: {weight*100:.1f}% | 獲配預算: {allocated_budget:,.0f} 元 | 預計購買零股: {calc_shares} 股")
    ...
    if new_sim_buys:
        ...
        print(f"\n📝 [模擬記帳] 成功通過自適應體檢並分配資金建倉：{', '.join([r['stock_name'] for r in new_sim_buys])}")
    elif candidate_buys:
        print("\n📭 今日有合格個股，但因在倉部位已卡死資金，或分配金額小於底線，故無任何新股建倉。")
