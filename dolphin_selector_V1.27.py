...
# ====================================================================
# 25.80 參數設定區 (歷史戰績自動清算 × 因果變數隔離 × 時空分流閘門)
# ====================================================================
VOLUME_FILTER = 500        
...
HISTORY_LEDGER_FILE = r"D:\Python-Training\N100\海豚選股法\dolphin_history_ledger.csv" # 👉 25.80 新增：歷史戰績大帳本路徑
...
def update_and_print_portfolio(api, today_str):
    ...
    for idx, row in df_pf.iterrows():
        ...
        if max_price >= target_tp_price and current_price <= dynamic_lock_price:
            ...
            log_to_history_ledger(row, current_price, net_profit, profit_percent, f"移動鎖利({tp_dr*100:.1f}%)") # 👉 自動清算
            continue 
            
        # 2️⃣ 25.80 停損出場：攔截並歸檔至大帳本
        if active_stop_loss_value > 0.0 and current_price < active_stop_loss_value:
            ...
            print(exit_msg)
            exit_p_rows.append(exit_msg)
            log_to_history_ledger(row, current_price, net_profit, profit_percent, f"跌破均線({target_ma_line})") # 👉 自動清算
            continue 
...
<nav class="navbar navbar-dark px-4 py-3">
    <span class="navbar-brand mb-0 h1 fs-3">🐬 海豚量化自適應指揮官儀表板 <small class="fs-6 text-muted-custom">v25.80 戰績結算版</small></span>
...
async def main():
    print("====================================================")
    print("🚀 海豚選股 25.80：[自動歷史戰績結算完全體] 啟動...")
    print("====================================================")
