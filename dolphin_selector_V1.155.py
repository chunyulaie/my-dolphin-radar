...
# ==========================================
# 25.5 參數設定區 (終極多頭防線·格式精簡版)
# ==========================================
VOLUME_FILTER = 500        # 單日成交量 500 張防線 

# 🎯 流派一：【起飆股】(左側壓縮埋伏) 
MA_SPREAD_LIMIT = 0.035    # 均線糾結度 3.5% 以內
BB_COMPRESS_LIMIT = 0.18   # 布林通道壓縮防線 (已對齊手機三竹，放寬至18%以內)
...
async def fetch_union_pyramid_pool():
    ...
    # 🛠️ 暫時改成這樣（開啟有頭模式）：
    browser = await launch(headless=False, userDataDir='./pyppeteer_cache', args=['--no-sandbox', '--disable-setuid-sandbox'])
    try:
...
async def main():
    print("====================================================")
    print("🚀 海豚選股 25.5：[終極多頭防線·格式精簡版] 啟動...")
    print("====================================================")
    ...
    for stock in STOCK_POOL:
        try:
            ...
            if not is_breakout_active:
                ...
                # 🎯 【功能優化二】強勢多頭排列防線：5MA 必須 >= 10MA >= 20MA，當場擊殺元太這種下垂死魚股
    ...
    if final_breakout_list or final_ambush_list:
        report_text = f"🐬 海豚選股 25.5 [終極多頭防線完全體] 🐬\n"
        ...
        report_text += "💡 提示：已啟動均線多頭防線（5MA >= 10MA >= 20MA）與米字號特殊股過濾機制。"
