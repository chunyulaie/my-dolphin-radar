...
# ====================================================
# 25.20 參數設定區 (AI 預回測過濾 · 自適應停利監控完全體)
# ====================================================
VOLUME_FILTER = 500        
...
def run_pre_backtest(api, stock_id):
    ...
    try:
        ...
        if df_raw.empty or len(df_raw) < 35: return False
        ...
        return total_net_profit >= 0  # 👉 只有利潤大於等於 0 元才算過關
    except:
        return False
...
def update_and_print_portfolio(api, today_str):
    ...
    survived_rows = []   
    report_p_rows = []   
    exit_p_rows = []     # 🎯 儲存今日觸發「出場與停利通知」的文字區
    ...
    for idx, row in df_pf.iterrows():
        ...
        # 1️⃣ 檢查是否達到「移動停利點」：通知但「保留持股不出場」
        if max_profit_percent >= tp_th and current_price <= (max_price * (1 - tp_dr)):
            exit_msg = f"🎉 停利：{sid} {sname} 曾最高創下 {max_price}，今日收 {current_price} 已自高點回檔達 {tp_dr*100:.1f}%！已滿足專屬停利條件！目前浮動獲利: {sign}{net_profit}元 ({sign}{profit_percent:.2f}%)"
            print(exit_msg)
            exit_p_rows.append(exit_msg)
            is_tp_triggered = True # 註記已達標，但依然會被放進生存名單中
            
        # 2️⃣ 檢查是否跌破 20MA 月線：強制硬砍出場
        if today_20ma > 0.0 and current_price < today_20ma:
            exit_msg = f"⚠️ 停損：{sid} {sname} 昨收 {current_price} 跌破20MA({today_20ma:.2f})！零股 {shares} 股強制結算！最終損益: {sign}{net_profit}元 ({sign}{profit_percent:.2f}%)"
            print(exit_msg)
            exit_p_rows.append(exit_msg)
            continue # 直接砍單，跳過不加入存活名單
        # ----------------------------------------------------
            
        # 沒死就留下來，並且印出大戶風格報表
        tp_status_tag = " 🔥(已達停利點!)" if is_tp_triggered else ""
        color_stars = "📈" if net_profit >= 0 else "📉"
        p_msg = f"{color_stars} {sid} {sname} | 買入: {b_date} | 成本: {b_price} ({shares}股) -> 現價: {current_price} [高點: {max_price}]{tp_status_tag} | 損益: {sign}{net_profit}元 ({sign}{profit_percent:.2f}%)"
        print(p_msg)
        report_p_rows.append(p_msg)
        
        # 確保最新的 max_price、與可能存在的自適應參數欄位被完整保留寫回 CSV
...
async def main():
    print("====================================================")
    print("🚀 海豚選股 25.20：[AI 預回測汰弱留強完全體] 啟動...")
    print("====================================================")
                        ...
                        if run_pre_backtest(api, stock):
                            ...
                            if calc_shares > 0:
                                ...
                                new_sim_buys.append({
                                    ...
                                    "max_price": latest_close  # 初始化歷史高點
                                })
