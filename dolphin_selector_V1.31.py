...
INIT_POOL_BUDGET = 250000    
MIN_STOCK_BUDGET = 15000     
...
def run_pre_backtest(api, stock_id):
    ...
    try:
        ...
        return max(0, max_possible_profit) 
    except:
        return 0
...
def update_and_print_portfolio(api, today_str):
    ...
    print("\n====================================================")
    print(f"💼 📊 【海豚選股 · 模擬帳戶當前資產即時回報 (動態複利明細版)】")
    print("====================================================")
    ...
    for idx, row in df_pf.iterrows():
        ...
        if today_high > max_price:
            max_price = today_high
            
        dynamic_lock_price = round(max_price * (1 - tp_dr), 2)
        
        if max_price >= target_tp_price and current_price <= dynamic_lock_price:
            exit_msg = f"🎉 鎖利通知：{sid} {sname} 曾創高點 {max_price} (盤中已過起跑線 {target_tp_price})，今日收盤 {current_price} 已跌破鎖利線 {dynamic_lock_price}！目前波段淨利: {sign}{net_profit}元 ({sign}{profit_percent:.2f}%)"
            print(exit_msg)
...
<div class="container-fluid p-4">
    ...
    html_content += """
            ...
            <div class="card mt-4">
                <div class="card-header fs-5 text-warning">🔍 AI 優化器：2年時空基因回測明細紀錄</div>
        ...
        for idx, p in enumerate(portfolio_data):
            ...
            <div class="accordion-item bg-transparent border-secondary">
                <h2 class="accordion-header" id="heading_{sid}">
                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_{sid}" aria-expanded="false" aria-controls="collapse_{sid}">
                        📊 {sid} {sname} — 2年最佳化策略回測歷史明細紀錄 (共 {len(records)} 筆回撤單交易)
                    </button>
                </h2>
            ...
            if not records:
                ...
                for r_idx, r in enumerate(records):
                    ...
                    r_sign = "+" if r_prof >= 0 else ""
                    r_class = "text-taiwan-red" if r_prof >= 0 else "text-taiwan-green"
                    
                    html_content += f"""
                                    <tr>
                                        <td>{r_idx + 1}</td>
                                        <td>{r.get("buy_date", "無")}</td>
                                        <td>{r.get("buy_price", 0)}</td>
                                        <td>{r.get("sell_date", "無")}</td>
                                        <td>{r.get("sell_price", 0)}</td>
                                        <td class="{r_class} fw-bold">{r_sign}{r_prof:,.0f}元</td>
                                        <td class="{r_class} fw-bold">{r_sign}{r_pct:.2f}%</td>
...
"""
    ...
    print(f"🖥️  [儀表板升級] 頂級工控基因摺疊 HTML 儀表板已成功生成於：{HTML_OUTPUT_FILE}")
...
async def main():
    ...
    if os.path.exists(HISTORY_LEDGER_FILE):
        try:
            ...
            if not df_ledger.empty and "net_profit" in df_ledger.columns:
                historical_net_sum = df_ledger["net_profit"].sum()
        except:
            pass
    ...
    print(f"💰 [複利總資產池] 當前動態滾動總額: {DYNAMIC_POOL_BUDGET:,.0f} 元")
    print(f"💰 [複利總資產池] 今日剩餘可用現金子彈: {available_cash:,.0f} 元")
                ...
                if df.iloc[-1]["close"] >= max(df.iloc[-1]["5MA"], df.iloc[-1]["10MA"], df.iloc[-1]["20MA"]):
                    ...
                    if triggered_days_ago == 0 and stock not in sim_purchased_stocks and stock not in REMOVE_LIST:
                        ...
                        if hist_profit > 0:
                            ...
                            print(f"✅ [體檢過關] {display_title} 兩年歷史淨利：{hist_profit} 元")
                        else:
                            print(f"❌ [體檢失敗] {display_title} 兩年歷史回測虧損！直接封殺！")
                    ...
                    if today_spread <= MA_SPREAD_LIMIT:
                        ...
                        if star_count == 3 and stock not in sim_purchased_stocks and stock not in REMOVE_LIST:
                            ...
                            if hist_profit > 0:
                                ...
                                print(f"❌ [體檢失敗] {display_title} 兩年歷史回測虧損！拒慢建倉！")
    ...
    if candidate_buys and available_cash > MIN_STOCK_BUDGET:
        ...
        print(f"\n⚖️  [動態配比] 今日合格新股：{len(candidate_buys)} 檔，總回測評分：{total_score}")
        ...
        for c in candidate_buys:
            ...
            if allocated_budget < MIN_STOCK_BUDGET:
                print(f"⚠️ {c['stock_id']}{c['stock_name']} 分配金額 {allocated_budget:,.0f} 低於限制，放棄建倉。")
                continue
            ...
            if calc_shares > 0:
                ...
                print(f"💰 [權重配發] 標的: {c['stock_id']} {c['stock_name']} | 獲配預算: {allocated_budget:,.0f} 元 | 預計零股: {calc_shares} 股")
    ...
    try:
        ...
        print(f"⚠️ [因供攔截] 自動導入優化器失敗（錯誤: {opt_err}）。")
