...
# ====================================================================
# 25.27 參數設定區 (AI 預回測體檢 · 目標起跑 × 專屬拉回自適應完全體)
# ====================================================================
VOLUME_FILTER = 500        
...
GLOBAL_TP_THRESHOLD = 0.15   # 預設預期獲利 15% 起跑
GLOBAL_TP_DROP = 0.03        # 預設高點拉回 3% 鎖利
    ...
    try:
        ...
        for i in range(35, len(df)):
            ...
            if in_pos:
                ...
                if max_p_pct >= GLOBAL_TP_THRESHOLD:
...
def update_and_print_portfolio(api, today_str):
    ...
    for idx, row in df_pf.iterrows():
        ...
        # 2️⃣ 檢查現價是否「跌破自適應波段停損線」 (硬性強制砍單離場)
...
async def main():
    print("====================================================")
    print("🚀 海豚選股 25.27：[目標價起跑 × 專屬拉回完全體] 啟動...")
    print("====================================================")
    ...
    if final_breakout_list or final_ambush_list or exit_report_text:
        report_chunks = []
        report_chunks.append(f"🐬 海豚選股 25.27 [雙向自適應移動鎖利完全體] 🐬")
