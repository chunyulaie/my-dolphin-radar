...
def update_and_print_portfolio(api, today_str):
    ...
    # 🎯 【修正核心一】：現價必須真的去抓「今天最新」的市價，不能被雷達最後一天綁架
    ...
    for idx, row in df_pf.iterrows():
        ...
        try:
            # 這裡用真正的日曆今天（real_today_str）去抓最新價格與20MA
        ...
        if today_20ma > 0.0 and current_price < today_20ma:
            exit_msg = f"⚠️ [自動停損結算] {sid} {sname} 跌破20MA({today_20ma:.2f})！零股 {shares} 股強制出場！最終損益: {sign}{net_profit}元 ({sign}{profit_percent:.2f}%)"
            print(exit_msg)
            report_p_rows.append(exit_msg)
            continue 
...
async def main():
    print("====================================================")
    print("🚀 海豚選股 25.11：[時空鎖死·跨日帳簿完全體] 啟動...")
    print("====================================================")
        ...
        try:
            ...
            if triggered_days_ago is not None:
                if df.iloc[-1]["close"] >= max(df.iloc[-1]["5MA"], df.iloc[-1]["10MA"], df.iloc[-1]["20MA"]):
                    ...
                    # 🎯 【修正核心二】：在建倉時，buy_date 鎖定在雷達數據的那天
    ...
    if final_breakout_list or final_ambush_list:
        report_text = f"🐬 海豚選股 25.11 [跨日資產累積完全體] 🐬\n"
