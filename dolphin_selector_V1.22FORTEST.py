...
# 🎯 【方案A：移動鎖利核心參數】
TP_THRESHOLD = 0.10        # 👉 獲利必須超過 10%，移動鎖利雷達才正式開機
TP_TRAILING_DROP = 0.05    # 👉 從歷史最高點拉回 5%，立刻手起刀落主動停利
...
def update_and_print_portfolio(api, today_str):
    ...
    survived_rows = []   
    report_p_rows = []   
    exit_p_rows = []     # 儲存今日觸發出場（停損或移動停利）的通報文字
    ...
    for idx, row in df_pf.iterrows():
        ...
        # 🎯 如果是舊資料可能沒有 max_price，預設至少等於買入價
        ...
        if current_price > max_price:
            ...
            row["max_price"] = max_price # 同步寫入當前列資訊
        ...
        # 🎯 【移動出場核心邏輯判定】
        # ----------------------------------------------------
        # 檢查從買進以來的歷史最大報酬率（用歷史最高價來算）
        max_profit_percent = ((max_price - b_price) / b_price)
        
        # 條件一：觸發【方案A：高點拉回移動鎖利】
        if max_profit_percent >= TP_THRESHOLD and current_price <= (max_price * (1 - TP_TRAILING_DROP)):
            exit_msg = f"🎉 停利：{sid} {sname} 曾最高創下 {max_price}，今日收 {current_price} 自高點回檔達 {TP_TRAILING_DROP*100:.0f}%！零股 {shares} 股主動鎖利出場！最終波段獲利: {sign}{net_profit}元 ({sign}{profit_percent:.2f}%)"
            print(exit_msg)
            exit_p_rows.append(exit_msg)
            continue # 自動剔除，不加入存活名單
        ...
        p_msg = f"{color_stars} {sid} {sname} | 買入: {b_date} | 成本: {b_price} ({shares}股) -> 現價: {current_price} [波段高點: {max_price}] | 損益: {sign}{net_profit}元 ({sign}{profit_percent:.2f}%)"
        print(p_msg)
        report_p_rows.append(p_msg)
        
        # 確保更新後的 max_price 一起被保留寫回 CSV
...
async def main():
    print("====================================================")
    print("🚀 海豚選股 25.14：[高點拉回移動鎖利完全體] 啟動...")
    print("====================================================")
                    ...
                    if triggered_days_ago == 0 and stock not in sim_purchased_stocks and stock not in REMOVE_LIST:
                        ...
                        if calc_shares > 0:
                            ...
                            new_sim_buys.append({
                                ...
                                "max_price": latest_close # 🎯 新增初始最大價欄位
                            })
    ...
    if final_breakout_list or final_ambush_list or exit_report_text:
        report_chunks = []
        report_chunks.append(f"🐬 海豚選股 25.14 [移動鎖利完全體] 🐬")
