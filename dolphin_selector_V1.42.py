...
# --- [指揮官極致海豚意念參數] ---
MA_SPREAD_LIMIT = 0.035        # 均線糾結門檻 (3.5%)
BB_COMPRESS_LIMIT = 0.10       # 布林帶寬極致壓縮門檻 (10%)
# ---------------------------------
...
# 資安優化：改由環境變數讀取金鑰
LINE_ACCESS_TOKEN = os.getenv('LINE_ACCESS_TOKEN', 'uyt/NqkAS3yCOhUAWGqey5HYGBe5mfct1n5MB1OQaV8Y1/X8HoypqNBwq/LOVXk5YnCknVCi8LEE5KZTXkbXT2V0CpOCAk0C/YRPJRA3Z2RREefQjAG41UQV0pbp1YQCnewazDskTwrpBsxHwRo4OQdB04t89/1O/w1cDnyilFU=')
TARGET_USER_ID = os.getenv('TARGET_USER_ID', 'Uf8818996f2c5846640e0ae8ae0360a72')
...
def update_and_print_portfolio(api, today_str):
    ...
    # 修正：實戰判斷的基準線應與下方 is_real_battle_window 的 14:00 對齊
    current_datetime = datetime.datetime.now()
    real_today_str = current_datetime.date().strftime("%Y-%m-%d") if current_datetime.time() >= datetime.time(14, 0, 0) else (current_datetime.date() - datetime.timedelta(days=1)).strftime("%Y-%m-%d")
    ...
    for idx, row in df_pf.iterrows():
        ...
        try:
            ...
            # [核心優化] 抓取最後一筆K線日期，判斷今天是否有真實交易數據更新
            latest_k_date = str(df_now.iloc[-1]["date"])[:10]
            is_data_updated_today = (latest_k_date == real_today_str)
            ...
            current_spread_pct = ((max(today_ma) - min(today_ma)) / df_now.iloc[-1]["20MA"]) * 100
            current_bb_width_pct = df_now.iloc[-1]["BB_Width"] * 100
            current_macd_val = df_now.iloc[-1]["MACD"]
            
            star_count = 0
            if current_spread_pct <= (MA_SPREAD_LIMIT * 100):
                star_count += 1
                if current_bb_width_pct <= (BB_COMPRESS_LIMIT * 100):
                    star_count += 1
                if current_macd_val > 0:
                    star_count += 1
            
            star_str = "⭐" * star_count if star_count > 0 else "🚀"
            macd_status = f"<span class='text-taiwan-red'>水上({current_macd_val:+.2f})</span>" if current_macd_val > 0 else f"<span class='text-taiwan-green'>水下({current_macd_val:.2f})</span>"
            bb_status = f"<span style='color: #00f2fe; font-weight:bold;'>{current_bb_width_pct:.1f}%</span>" if current_bb_width_pct <= 10.0 else f"{current_bb_width_pct:.1f}%"
            
            strategy_stars = f"{star_str} | 糾結:{current_spread_pct:.1f}% | 帶寬:{bb_status} | MACD:{macd_status}"
        ...
        # [防禦攔截] 只有在確認今天有開盤且資料更新的狀況下，才允許觸發移動鎖利
        if is_data_updated_today and max_price >= target_tp_price and current_price <= dynamic_lock_price:
        ...
        # [防禦攔截] 加上 is_data_updated_today 條件，阻斷假日的錯覺實戰判定
        is_real_battle_window = is_data_updated_today and (datetime.time(14, 0, 0) <= current_time <= datetime.time(23, 59, 0))
        ...
        if is_currently_broken and active_stop_loss_value > 0.0:
            if is_real_battle_window:
                ...
                reason_str = "非實戰考核時間(14:00-23:30)" if is_data_updated_today else "今日為假日或休市(無更新資料)"
                exit_p_rows.append(f"🔬 [沙盒測試] {sid} {sname} 技術型態上跌破 {target_ma_line}，但{reason_str}，鎖定在倉防禦。")
...
<head>
    ...
    <style>
        ...
        .star-tag {{ color: #a1a5b7; font-size: 0.85rem; font-weight: 500; letter-spacing: 0.5px; }}
    </style>
</head>
<body>
<nav class="navbar navbar-dark px-4 py-3">
    <span class="navbar-brand mb-0 h1 fs-3">🐬 海豚量化自適應指揮官儀表板 <small class="fs-6 text-muted-custom">v1.41 防誤判強護盾版</small></span>
            ...
            html_content += f"""
                            <tr>
                                <td>
                                    ...
                                    <span class="star-tag">{p['strategy_stars']}</span><br>
                    ...
                    <div class="accordion-body">
                        <div class="table-responsive">
                            <table class="table table-sm table-dark table-hover mb-0 text-center small">
                                ...
                                <tbody>
            """
            if not records: html_content += f'<tr><td colspan="9" class="text-muted-custom py-3">⚠️ 未偵測到明細資料，請確認 API 狀態 or 回測引擎是否順利完成。</td></tr>'
            else:
...
async def main():
    ...
    for stock in STOCK_POOL:
        try:
            ...
            # [核心優化] 計算今天距實際 K 線最新日期的「天數差」
            # 若為週末或假日跑排程，days_diff > 0，將被攔截阻止重複觸發建倉
            today_k_date = str(today_k["date"])[:10]
            try:
                days_diff = (datetime.datetime.strptime(today_str, "%Y-%m-%d").date() - datetime.datetime.strptime(today_k_date, "%Y-%m-%d").date()).days
            except:
                days_diff = 0
            
            today_change_pct = ((today_k["close"] - yesterday_k["close"]) / yesterday_k["close"]) * 100
            
            # 只有在最新K線是「今天」時，才觸發今日漲停鎖死攔截
            if days_diff == 0 and today_change_pct >= 9.8 and today_k["close"] == today_k["max"]:
            ...
            if triggered_days_ago is not None and df.iloc[-1]["close"] >= max(df.iloc[-1]["5MA"], df.iloc[-1]["10MA"], df.iloc[-1]["20MA"]):
                # 結合實際天數差：避免週末將週五的發動當成「0天」發動
                actual_days_ago = triggered_days_ago + days_diff
                raw_breakout_data.append({"stock_id": stock, "stock_name": c_name, "title": display_title, "days_ago": actual_days_ago, "spread": bo_spread, "bb": bo_bb, "close": latest_close})
                is_bo_active = True 
                
                # 只有 "絕對天數 == 0" (代表今天才發生) 才會加入實戰建倉評估
                if actual_days_ago == 0:
                    ...
                    candidate_buys.append({"stock_id": stock, "stock_name": c_name, "latest_close": latest_close, "buy_type": "正飆(0天)", "buy_date": today_k_date, "score": safe_score, "stars_display": "🚀 真·正飆破"})
            ...
            if not is_bo_active and df.iloc[-1]["5MA"] >= df.iloc[-1]["10MA"] >= df.iloc[-1]["20MA"]:
                ...
                if (max(today_ma) - min(today_ma)) / df.iloc[-1]["20MA"] <= MA_SPREAD_LIMIT:
                    ...
                    # 只有在最新K線是「今天」時，才納入實戰建倉評估
                    if stars_count == 3 and days_diff == 0:
                        ...
                        if b_recs:
                            for r in b_recs: r["stock_id"] = str(stock).strip(); GLOBAL_RADAR_RECORDS.append(r)
                        safe_score = hist_p if hist_p > 0 else 1 
                        candidate_buys.append({"stock_id": stock, "stock_name": c_name, "latest_close": latest_close, "buy_type": "3星起飆", "buy_date": today_k_date, "score": safe_score, "stars_display": "⭐⭐⭐ 滿星潛伏"})
        except Exception as e:
    ...
    if exit_text.strip():
        ...
        for line in exit_text.split('\n'):
            ...
            # [核心攔截] 加上了 "防線警戒"，確保所有重大行動皆會收到推播，但假日不會被轟炸
            if line_clean and ("鎖利通知" in line_clean or "停損出場" in line_clean or "防線警戒" in line_clean):
                exit_rows.append(line_clean)
                
        if exit_rows:
            line_report_chunks.append("💀【在倉老兵即時清倉離場 / 警戒通知】")
            for er in exit_rows:
