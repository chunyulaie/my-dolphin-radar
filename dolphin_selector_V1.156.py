...
# ==========================================
# 25.6 參數設定區 (量能純度防線·專殺股災雜魚版)
# ==========================================
VOLUME_FILTER = 500        # 當日成交量基本防線：500 張
VOLUME_5MA_FILTER = 400    # 🎯【量能純度防線】5日平均成交量需 > 400 張，徹底封殺平時沒量的殭屍暴量股

# 🎯 流派一：【起飆股】(左側壓縮埋伏) 
MA_SPREAD_LIMIT = 0.035    # 均線糾結度 3.5% 以內
BB_COMPRESS_LIMIT = 0.18   # 布林通道壓縮防線 (已對齊手機三竹，18%以內)
...
async def main():
    print("====================================================")
    print("🚀 海豚選股 25.6：[量能純度防線·雜魚殺手版] 啟動...")
    print("====================================================")
    ...
    for stock in STOCK_POOL:
        try:
            ...
            df["5MA_Vol"] = df["volume"].rolling(window=5).mean() # 5日均量
            ...
            # 🎯【功能優化】當日成交量 > 500張 且 5日均量 > 400張，雙重封殺「一日股災暴量股」
            ...
            if not is_breakout_active:
                ...
                if f_5ma >= f_10ma >= f_20ma:
                    # 🛠️ 修正後：拿三條均線的最高減最低，除以月線(20MA)，最精準反映成本糾結，不受當天股價暴漲影響！
    ...
    if final_breakout_list or final_ambush_list:
        report_text = f"🐬 海豚選股 25.6 [量能純度防線版] 🐬\n"
        ...
        report_text += "💡 提示：已啟動均線多頭排列防線。新增5日均量防線（5MA_Vol > 400張），徹底排除單日股災虛胖暴量股。"
